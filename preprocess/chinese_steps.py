...
- final subset
- length filter
- (optional) post-length subset/balance
- save cleaned.jsonl
"""
...
def _assert_required_cols(df: pd.DataFrame, cols: List[str], *, stage: str) -> None:
    ...
    if missing:
        raise ValueError(f"Missing required columns at stage={stage}: {missing}. Columns={list(df.columns)}")
...
# =====================================================================
# Step 8: two-stage prompt filter
# =====================================================================
...
def load_and_process_chinese(merged_jsonl_path: str, text_cfg: Dict[str, Any]) -> pd.DataFrame:
    ...
    # (F) OPTIONAL: post-length subset/balance (only if explicitly enabled)
    if bool(text_cfg.get("post_length_subset", False)):
        df = apply_subset(df, text_cfg)
        print(f"[INFO] After post-length subset: {len(df)} samples remaining.")
        if not df.empty:
            print("[INFO] Label distribution after post-length subset:\n", df["Diagnosis"].value_counts())
        _assert_no_unknown(df, stage="post_length_subset")
